...
def f(t,s_1,s_2,s_3,s_1p=np.zeros(3),s_2p=np.zeros(3),s_3p=np.zeros(3)):
    ...
    if r_12n < 0.01 or r_13n < 0.01:
        print("Collision!")
        jitter=np.random.normal(loc=y_1)
        v_1 = 1.5*jitter
        v_2 = -v_2 + jitter
        v_3 = -v_3 + jitter
        y_1_p = t*v_1
    ...
    alpha = np.random.normal(1)
    v_1 += t*G*(m_2*r_13n**2 + m_3*r_12n**2) / (r_12n*r_13n)**2 * (r_12 + r_13 - r_23)
    v_2 += t*G*(m_1*r_23n**2 + m_3*r_12n**2) / (r_12n*r_23n)**2 * (r_12 + r_23 - r_13)
    v_3 += t*G*(m_2*r_13n**2 + m_1*r_23n**2) / (r_23n*r_13n)**2 * (r_13 + r_23 - r_12)
    y_1_p = t*v_1
    y_2_p = t*v_2
    y_3_p = t*v_3
...
print(np.shape(x_1))
ax.plot(x_1[:,0],x_1[:,1],x_1[:,2],label=f'first mass {m_1}',linestyle='--')
ax.plot(x_2[:,0],x_2[:,1],x_2[:,2],label=f'second mass {m_2}',linestyle='--')
ax.plot(x_3[:,0],x_3[:,1],x_3[:,2],label=f'third mass {m_3}',linestyle='--')
...
ax.scatter(x_1[-1,0],x_1[-1,1],x_1[-1,2],label=f'first terminal point',color='blue')
ax.scatter(x_2[-1,0],x_2[-1,1],x_2[-1,2],label=f'second terminal point',color='orange')
